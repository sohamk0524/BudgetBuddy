...
def upsert_gamification(user_id: str, **kwargs) -> datastore.Entity:
    ...
    if existing:
        ...
        entity = datastore.Entity(key=key, exclude_from_indexes=['weekly_challenge_json', 'challenge_history_json'])
        entity['user_id'] = user_id
        entity['challenges_enabled'] = True
        entity['challenge_history_json'] = '[]'
        entity['created_at'] = datetime.utcnow()
    entity.update(kwargs)
    entity['updated_at'] = datetime.utcnow()
    client.put(entity)
